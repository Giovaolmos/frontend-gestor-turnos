      ...
      prev.map((day) =>
        day.dayOfWeek === dayOfWeek
          ? { 
              ...day, 
              isOpen: !day.isOpen,
              slots: !day.isOpen && day.slots.length === 0 
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold sm:text-3xl">Horarios de atención</h1>
              ...
              <CardHeader className="pb-2 sm:pb-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 sm:gap-4">
                    ...
                    />
                    <CardTitle className={`text-base sm:text-lg ${!daySchedule.isOpen ? 'text-muted-foreground' : ''}`}>
                      {label}
                    </CardTitle>
                  </div>
                  {daySchedule.isOpen && (
                ...
                {!daySchedule.isOpen && (
                  <CardDescription className="text-xs sm:text-sm">Cerrado</CardDescription>
                )}
              </CardHeader>
                        ...
                        >
                          <div className="flex flex-1 flex-col gap-2 xs:flex-row xs:items-center">
                            <div className="flex items-center gap-1.5 sm:gap-2">
                              <Label className="w-12 text-xs sm:w-16 sm:text-sm">Desde:</Label>
                              <Input
                                type="time"
                                value={slot.start}
                                onChange={(e) =>
                                  handleSlotChange(value, slotIndex, 'start', e.target.value)
                                }
                                className="w-24 text-sm sm:w-32"
                              />
                            </div>
                            <div className="flex items-center gap-1.5 sm:gap-2">
                              <Label className="w-12 text-xs sm:w-16 sm:text-sm">Hasta:</Label>
                              <Input
                                type="time"
                                value={slot.end}
                                onChange={(e) =>
                                  handleSlotChange(value, slotIndex, 'end', e.target.value)
                                }
                                className="w-24 text-sm sm:w-32"
                              />
                            </div>
